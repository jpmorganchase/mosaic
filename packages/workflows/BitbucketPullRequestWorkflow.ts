...
async function createPullRequest(
  ...
  filePath: string,
  { user, markdown, frontmatter }: BitbucketPullRequestWorkflowData
) {
  const {
    credentials,
    remote,
  ...
  // rationale — this is the same contract.
  let nextMeta: Record<string, unknown> = metadata as Record<string, unknown>;
  if (typeof frontmatter === 'string') {
    try {
      const parsed = matter(`---\n${frontmatter}\n---\n`).data ?? {};
      nextMeta = parsed as Record<string, unknown>;
  ...
  const updatedPage = { ...nextMeta, content: markdown };
