...
async function doesPreviousCloneExist(repo, dir, branch) {
    try {
        ...
        if (projectURI === repo) {
            const existingBranchName = (await spawn('git', ['rev-parse', '--abbrev-ref', 'HEAD'], dir)).trim();
...
export default class Repo {
    ...
    async onCommitChange(callback, errCallback, disableAutoPullChanges = false, updateInterval = 15 * 60 * 1000) {
    ...
    async diff(latestRevision) {
        ...
        const diff = await spawn('git', ['diff', `${latestRevision}..${this.#branch}`, '--name-status', '--no-renames'], this.#dir);
        if (!diff) {
            return [];
        }
        return diff
    ...
    async currentLocalRevision() {
        ...
        if (!result) {
            console.warn(`No revision found for 'HEAD' when running \`git rev-parse HEAD\` in '${this.#dir}' for '${this.#name}'`);
            return '';
        }
    ...
    async latestRemoteRevision() {
        ...
        if (!result) {
            console.warn(`No revision found for tag '${this.#branch}' when running \`git rev-parse ${this.#branch}\` in '${this.#dir}' for '${this.#name}'`);
            return null;
        }
        return result.trim();
    }
