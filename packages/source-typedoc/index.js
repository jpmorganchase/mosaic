...
export default async function init(options) {
    if (!process.env.MOSAIC_DOCS_CLONE_CREDENTIALS ||
        process.env.MOSAIC_DOCS_CLONE_CREDENTIALS.indexOf(':') === -1) {
        throw new Error('No valid credentials found in `BITBUCKET_CLONE_CREDENTIALS`. Ensure a valid *non-URL-encoded* env var is setup with the format: "SID:PersonalKey"');
    }
...
function createRootIndexDescriptors(contentRoot, options, repositoryAPI) {
    return getRootIndexRoute(contentRoot).map(async (pagePath) => {
        ...
        const descriptor = await createRootIndex(versionedContentRoot, pagePath, fullRoute, additionalMetadata);
        return descriptor
            ? {
                ...descriptor,
                route
    ...
    return getAllPageRoutes(contentRoot, docsPattern).map(async (pagePath) => {
        ...
        if (descriptor) {
            ...
            if (rootIndex === 'index.html') {
                ...
                const { date: releaseDate, description } = await repositoryAPI.getTagInfo(options.repoBranch);
...
async function syncRepoSource(options) {
    const { repositoryAPI } = await cloneAndSyncRepo(options, process.env.BITBUCKET_CLONE_CREDENTIALS);
    return {
...
async function watchRepo(options, repositoryAPI, callback, errCallback) {
    ...
    let unsubscribe;
    const getPageDescriptors = () => Promise.all([
    ...
    try {
        ...
        unsubscribe = await repositoryAPI.onCommitChange(async (gitUpdates) => {
        ...
        }, errCallback, false, checkIntervalMins * 60000);
        callback(await getPageDescriptors());
        return () => {
            unsubscribe();
        };
...
async function cloneAndSyncRepo(options, repoCredentials) {
    ...
    try {
        ...
        console.error(`======== Could not find folder '${clonedDocsSubfolder}' in repository ${repoUrl} (branch '${repoBranch}'). Check the 'subfolder' field in the CMS ========`);
        process.exit(1);
    }
    return {
        contentRoot,
