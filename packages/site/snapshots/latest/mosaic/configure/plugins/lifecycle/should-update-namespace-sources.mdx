...
sidebar:
  label: shouldUpdateNamespaceSources
  groupLabel: shouldUpdateNamespaceSources
  priority: 1
lastModified: 1780396138477.1199
fullPath: /mosaic/configure/plugins/lifecycle/should-update-namespace-sources.mdx
route: /mosaic/configure/plugins/lifecycle/should-update-namespace-sources
breadcrumbs:
  - label: Configure
...
navigation:
  ...
  next:
    title: $AliasPlugin
    route: /mosaic/configure/plugins/alias-plugin
    group: Plugins
sidebarData:
...
It is called every time *any* source emits new pages and should return a boolean to indicate if **other sources that share the same [source namespace](../../sources/index#source-namespace)** should re-run `afterUpdate`.
