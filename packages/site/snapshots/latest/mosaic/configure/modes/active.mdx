...
In `active{:bat}` mode content can be *pulled* from heterogeneous data sources and normalized via plugins, to the configured components/theme.
...
e.g Our sample docs are aggregated into a *namespace* called `mosaic` and served by the user journey `http://localhost:3000/mosaic{:bat}`
