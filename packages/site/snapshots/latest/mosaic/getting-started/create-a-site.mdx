...
sidebar:
  ...
  priority: 8
lastModified: 1781012102900.0388
...
You can copy the `site` directory from this repository into your project. The site directory is a standard Next.js App Router app that loads the packages created by this repo, with an additional `mosaic-config.mjs` file to configure sources and other settings.
...
> **Note: App Router layout.** The site uses Next.js App Router
> (`src/app/`), not the legacy Pages Router (`src/pages/`). Key files
> you'll touch when customising:
>
> * `src/app/layout.tsx` — root layout, global CSS, session seeding.
> * `src/app/providers.tsx` — client-side provider stack
>   (`SessionProvider`, theme, store, layout).
> * `src/app/[...route]/page.tsx` — the catch-all RSC route that runs
>   middleware and renders MDX.
> * `src/app/[...route]/MdxComponents.ts` — registry for MDX-visible
>   components (replaces the old `_app.tsx` component merge).
> * `src/auth.ts` — Auth.js v5 wiring (`handlers`, `auth`, `signIn`,
>   `signOut`).
>
> **Footgun:** the session must be resolved in `layout.tsx` with `await auth()` and passed to `<SessionProvider session={...}>` as a prop. Do
> not let `SessionProvider` enter its "loading" state during static
> generation — it races React's internal dispatcher and produces a
> non-deterministic `TypeError: Cannot read properties of null (reading 'useState')` crash. The reference `packages/site` already does this
> correctly; keep that pattern when you copy the directory.
...
5. [Build a static export](../configure/modes/static-export) for hosting on any static CDN with no Node runtime.
