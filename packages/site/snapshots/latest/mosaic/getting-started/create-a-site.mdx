...
breadcrumbs:
  ...
  - label: Create a Site
    path: /mosaic/getting-started/create-a-site.mdx
...
If you are setting up a site in an empty directory, run the following command to create a package.json file:

```bash
yarn init
```

Run the following command to generate a new Mosaic site:
...
3.  [Configure your own sources in the mosaic.config.js file to pull content from different locations.](../configure/sources/index)
